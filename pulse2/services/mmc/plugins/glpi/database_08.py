...
class Glpi08(DyngroupDatabaseHelper):
    ...
    def getMachineMac(self, uuid):
        ...
        query = session.query(Network).select_from(self.machine.join(self.network))
        query = self.filterOnUUID(query.filter(self.network.c.itemtype == 'Computer'), uuid)
        ret = unique([m.mac for m in query.all()])
        session.close()
        return ret
    ...
    def addEntity(self, session, entity_name, parent_id, comment):
        ...
        parent_entity = session.query(Location).filter_by(id=parent_id,).one()
    ...
    def addEntityRule(self, session, rule_data):
        ...
        rule.entities_id = 0
        rule.sub_type = 'PluginFusinvinventoryRuleEntity'
...
class Machine(object):
    ...
    def to_a(self):
        owner_login, owner_firstname, owner_realname = Glpi08().getMachineOwner(self)
        return [
            ['name',self.name],
