...
def read_logs(logfiles, start_date, stop_date):
    ...
    for file in logfiles:
        ...
        for line in fh:  # Parse each line in the log file
            ...
            if 'global' in dump:
                if launcher not in runnings:
                    runnings[launcher] = {}
                if stamp not in runnings[launcher]:
                    runnings[launcher][stamp] = int(dump['global']['running'])
                else:
                    runnings[launcher][stamp] += int(dump['global']['running'])
                if launcher not in zombies:
                    zombies[launcher] = {}
                if stamp not in zombies[launcher]:
                    zombies[launcher][stamp] = int(dump['global']['zombie'])
                else:
                    zombies[launcher][stamp] += int(dump['global']['zombie'])

            if 'loadavg' in dump:
                if launcher not in loads:
                    loads[launcher] = {}
                if stamp not in loads[launcher]:
                    loads[launcher][stamp] = int(dump['loadavg']['1min'])
                else:
                    loads[launcher][stamp] += int(dump['loadavg']['1min'])

            if 'fd' in dump:
                if launcher not in fds:
                    fds[launcher] = {}
                if stamp not in fds[launcher]:
...
if __name__ == "__main__":
    ...
    if zombies:
        ...
        for launch in launcher_list:
            for hour in launch_hours:
                if launch not in launch_zombies:
