...
if (isset($groupname) && $groupname != '' && !xmlrpc_group_name_exists($groupname)) {
    ...
    if (move_uploaded_file($_FILES['importfile']['tmp_name'], '/tmp/' . $_FILES['importfile']['name'])) {
        new NotifyWidgetSuccess(_T("File Uploaded", "dyngroup"));
    }
    else [
        new NotifyWidgetSuccess(_T("File not Uploaded", "dyngroup"));
    }
